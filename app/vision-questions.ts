...
export const visionQuestions: VisionQuestion[] = [
  ...
  essay("cnn-22", "CNN·이미지 모델", "사고형", "CNN이 이미지 처리에서 완전연결망보다 효율적인 이유와, 깊어질수록 수용영역과 특징의 성격이 어떻게 변하는지 100자 이상으로 설명하시오.", "CNN은 작은 필터의 국소 연결과 위치 전반의 가중치 공유로 공간 구조를 보존하면서 완전연결망보다 적은 파라미터로 패턴을 찾는다. 초기 층은 모서리와 질감 같은 국소 특징을 추출하고 층이 깊어질수록 여러 국소 영역이 결합되어 수용영역이 넓어진다. 그 결과 후반 층은 물체의 부분과 전체 형태처럼 더 추상적이고 전역적인 특징을 표현한다.", "답안에는 국소 연결, 가중치 공유, 파라미터 효율, 수용영역 증가, 저수준에서 고수준으로 바뀌는 계층적 특징이 모두 포함되어야 합니다."),
  essay("cnn-23", "CNN·이미지 모델", "고난도", "VGG, ResNet, MobileNet의 설계 목표와 핵심 장치를 비교하여 100자 이상으로 설명하시오.", "VGG는 3×3 합성곱을 규칙적으로 반복해 단순하고 깊은 특징 계층을 만들지만 파라미터와 계산량이 크다. ResNet은 F(x)+x의 shortcut으로 깊은 네트워크의 최적화와 degradation 문제를 완화하며 bottleneck으로 계산을 줄인다. MobileNet은 depthwise convolution과 1×1 pointwise convolution을 분리해 모바일 환경에서 연산량과 모델 크기를 크게 낮춘다.", "세 모델을 단순히 성능 순서로 쓰기보다 VGG의 작은 커널 반복, ResNet의 residual shortcut, MobileNet의 depthwise separable convolution과 효율성 목표를 비교해야 합니다."),
  essay("cnn-24", "CNN·이미지 모델", "고난도", "합성곱 모델의 계산 자원을 분석할 때 파라미터 수, activation memory, FLOPs를 구분해야 하는 이유를 100자 이상으로 설명하시오.", "파라미터 수는 저장할 가중치의 규모와 주로 관련되지만 실행 중 메모리는 중간 feature map인 activation 크기의 영향을 크게 받는다. FLOPs는 출력 공간의 각 위치에서 수행하는 커널 연산량을 나타내므로 속도와 연결되며, 파라미터가 적어도 큰 feature map에서는 계산이 많을 수 있다. 따라서 모델 크기, 학습 메모리, 실행 속도를 하나의 숫자로 대신 판단하면 안 된다.", "파라미터는 모델 저장량, activation은 실행·역전파 메모리, FLOPs는 계산량을 주로 나타냅니다. 세 값이 층별로 서로 다른 양상을 보인다는 점이 핵심입니다."),
  ...
  essay("vit-23", "ViT·학습 전략", "고난도", "훈련 loss는 계속 감소하지만 validation loss가 상승하기 시작했다. 적용할 전략과 판단 기준을 100자 이상으로 설명하시오.", "훈련과 검증 곡선이 벌어지는 시점은 과적합 신호이므로 검증 성능이 가장 좋은 checkpoint를 기준으로 early stopping을 적용할 수 있다. 동시에 weight decay나 dropout, label을 바꾸지 않는 data augmentation을 검토한다. 다만 test set은 모델 선택에 사용하지 않고 최종 선택이 끝난 뒤 한 번만 평가하며, 정규화 강도와 증강은 validation 성능으로 조정한다.", "핵심은 과적합 진단, early stopping, 정규화 또는 증강, validation 기반 선택, test set 격리입니다. 훈련 loss만 보고 계속 학습하는 답은 충분하지 않습니다."),
  essay("vit-24", "ViT·학습 전략", "고난도", "사전학습 이미지 모델을 새 분류 문제에 적용할 때 linear probing, fine-tuning, data augmentation, learning-rate scheduling을 어떻게 조합할지 100자 이상으로 설명하시오.", "먼저 사전학습 backbone을 고정하고 새 분류 head만 학습하는 linear probing으로 특징의 전이 가능성과 baseline을 확인한다. 데이터가 충분하고 추가 적응이 필요하면 낮은 학습률로 일부 또는 전체 backbone을 fine-tuning한다. crop, flip, 색 변화처럼 label을 보존하는 augmentation으로 과적합을 줄이고, learning-rate schedule로 초반의 빠른 적응과 후반의 세밀한 수렴을 함께 노린다.", "답안은 사전학습 가중치 활용, linear probing과 fine-tuning의 차이, label-preserving augmentation, 작은 학습률과 schedule의 목적을 연결해 설명해야 합니다."),
];
