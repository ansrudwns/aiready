...
import { hintExamTopics, hintExplanationNotes, hintSupplementQuestions } from "./hint-exam-questions";
...
const explanationNotes: Record<string, string> = {
  ...
  "ml-25": "예를 들어 스팸 분류에서는 메일 문장과 발신자 범주가 입력이고 스팸 여부가 정답입니다. 원본 데이터의 종류와 모델 안에서 맡는 역할을 구분하면 ‘숫자가 아니므로 feature가 아니다’라는 오해를 피할 수 있습니다.",
  "ml-26": "집값을 맞히면 지도학습 회귀, 스팸 여부를 맞히면 지도학습 분류입니다. 고객을 미리 정한 정답 없이 구매 패턴만으로 묶는 군집화와 비교하면 label이 제공되는지 여부가 분명해집니다.",
  "ml-27": "화재가 아닌데 경보가 울린 상황을 떠올리면 FP를 기억하기 쉽습니다. 반대로 실제 화재인데 경보가 울리지 않은 경우는 FN이며, 어느 실수가 더 위험한지에 따라 precision과 recall의 우선순위가 달라집니다.",
  "ml-28": "train과 test가 모두 나쁘면 모델이 너무 단순한 과소적합일 수 있고, train만 유난히 좋으면 과적합을 의심합니다. 대표성 있는 데이터, 정규화, 복잡도 제한과 교차검증은 이 차이를 줄이는 데 활용됩니다.",
  "ml-29": "교육연수가 같아도 소득은 개인의 직업·지역·우연한 상황 때문에 서로 다를 수 있습니다. 관측하지 않은 요인까지 X에 추가하면 일부 오차는 줄지만, 개별 결과를 완벽히 결정할 수 있다고 가정해서는 안 됩니다.",
  "ml-30": "precision이 0.8이고 recall이 0.5라면 F1은 약 0.62로 두 값의 중간보다 낮은 쪽에 더 끌립니다. 따라서 한 지표만 좋은 모델보다 두 지표가 고르게 유지되는 모델을 찾는 데 도움이 됩니다.",
  ...
  "nn-11": "회귀 결과표의 p-value는 계수가 0이라는 가정 아래 지금처럼 큰 추정값이 나올 가능성을 나타냅니다. 값이 0.05보다 충분히 작으면 해당 설명변수와 반응변수의 관계가 통계적으로 유의하다고 해석합니다.",
  ...
  "nn-18": "nn.Module에서 forward를 작성하면 model(x)처럼 모델 객체를 호출했을 때 __call__ 매직 메서드가 forward를 실행합니다. 그래서 일반적으로 forward를 직접 호출하기보다 model(x) 형태로 순전파를 실행합니다.",
  ...
  "nn-24": "설명변수를 추가하면 모델이 기존 예측식을 그대로 사용할 수 있으므로 훈련 데이터의 R²는 작아지지 않습니다. 그러나 새 변수가 실제로 도움이 되는지는 validation 또는 test 성능, 해석 가능성, 과적합 위험까지 함께 확인해야 합니다.",
  "nn-25": "실제값이 12이고 예측값이 10이면 잔차는 2이므로 모델이 2만큼 낮게 예측했습니다. 반대로 예측값이 15라면 잔차는 -3이며, 부호를 읽으면 과소예측과 과대예측 방향을 구분할 수 있습니다.",
  "nn-26": "잔차가 2와 -2라면 그대로 더한 값은 0이지만 두 예측이 정확했던 것은 아닙니다. 제곱하면 4+4=8이 되어 오차가 남으며, RSS를 표본 수로 나누면 MSE와 연결됩니다.",
  "nn-27": "제곱은 양수와 음수 잔차가 상쇄되는 일을 막고 큰 오차에 더 큰 벌점을 줍니다. 따라서 멀리 떨어진 이상치가 계수에 큰 영향을 줄 수 있다는 점도 최소제곱 결과를 해석할 때 함께 확인해야 합니다.",
  "nn-28": "학습은 loss 계산, backward를 통한 gradient 계산, optimizer의 파라미터 갱신 순서로 진행됩니다. 학습률은 이 마지막 이동 크기에 관여하며 데이터에서 자동으로 정답처럼 학습되는 weight와는 구분됩니다.",
  "nn-29": "full-batch는 방향이 안정적이지만 한 번 계산하는 비용이 크고, mini-batch는 한 걸음이 가볍지만 방향이 흔들릴 수 있습니다. 같은 epoch 수가 곧 같은 수렴 속도를 뜻하지 않으므로 loss 곡선과 실제 계산 시간으로 비교해야 합니다.",
  "nn-30": "X=0이 관측 범위 밖이라면 절편의 수학적 의미는 있어도 현실 해석은 어색할 수 있습니다. 예를 들어 관측 연령이 20세 이상인데 0세의 소득을 설명하려 하면 외삽이 되므로 해석 범위를 확인해야 합니다.",
  ...
  "llm-19": "합성 데이터는 양을 빠르게 늘릴 수 있지만 원 모델의 잘못된 답이나 편향도 함께 복제할 수 있습니다. 중복·저품질 데이터를 걸러내고 다양한 과제와 사람 검토를 함께 사용해야 데이터 오염과 품질 저하를 줄일 수 있습니다.",
...
const rawPracticeQuestionBank: Question[] = [
  ...
  Q("ml-25", "ML 기초·검증", "객관식", "기초", "머신러닝에서 feature가 될 수 있는 데이터에 대한 설명으로 옳은 것은?", "수치뿐 아니라 범주·텍스트·이미지·음성도 입력 feature가 될 수 있다.", "feature는 모델이 예측에 사용하는 입력 정보입니다. 원본 형태가 무엇이든 모델이 처리할 수 있는 수치 표현으로 변환하면 범주, 문장, 이미지, 음성도 feature로 사용할 수 있습니다.", undefined, ["수치뿐 아니라 범주·텍스트·이미지·음성도 입력 feature가 될 수 있다.", "정수와 실수만 feature가 되고 나머지는 모두 label로만 사용한다.", "이미지와 음성은 feature가 될 수 있지만 텍스트와 범주는 사용할 수 없다.", "feature는 학습 뒤 만들어진 weight와 bias만을 가리키는 이름이다."]),
  Q("ml-26", "ML 기초·검증", "객관식", "기초", "지도학습에 대한 설명으로 가장 적절한 것은?", "입력과 정답의 관계를 학습해 새 입력의 정답을 예측한다.", "지도학습 데이터에는 입력 feature와 목표 label이 함께 있습니다. 모델은 두 값의 관계를 학습해 새로운 입력에 대한 연속값 또는 범주를 예측합니다.", undefined, ["입력과 정답의 관계를 학습해 새 입력의 정답을 예측한다.", "정답 없이 데이터 사이의 거리만 계산해 비슷한 집단을 찾는다.", "보상 없이 임의 행동을 반복하며 모든 파라미터를 고정한다.", "측정오차를 완전히 제거한 뒤 정답과 입력의 관계를 삭제한다."]),
  Q("ml-27", "ML 기초·검증", "객관식", "핵심", "분류 혼동행렬의 FP(False Positive)를 올바르게 설명한 것은?", "실제는 음성이지만 모델이 양성으로 예측한 경우", "혼동행렬은 분류의 실제 class와 예측 class를 비교합니다. FP는 양성이라고 예측했지만 실제로는 음성인 오탐이며, Type I error라고도 부릅니다.", undefined, ["실제는 음성이지만 모델이 양성으로 예측한 경우", "실제는 양성이며 모델도 양성으로 예측한 경우", "실제는 양성이지만 모델이 음성으로 예측한 경우", "실제는 음성이며 모델도 음성으로 예측한 경우"]),
  Q("ml-28", "ML 기초·검증", "객관식", "핵심", "과적합 모델에서 흔히 관찰되는 train과 test 오류의 관계는?", "train 오류는 작지만 test 오류는 상대적으로 크다.", "과적합은 훈련 데이터의 세부와 잡음까지 맞춘 상태입니다. train 성능은 좋지만 새로운 test 데이터에 일반화하지 못해 두 오류 사이의 차이가 커집니다.", undefined, ["train 오류는 작지만 test 오류는 상대적으로 크다.", "train 오류와 test 오류가 모두 매우 큰 상태만 나타난다.", "train 오류는 크지만 test 오류는 항상 정확히 0이 된다.", "train 오류와 test 오류가 언제나 같은 값으로 고정된다."]),
  Q("ml-29", "ML 기초·검증", "객관식", "사고형", "Y=f*(X)+ε에서 모델을 충분히 학습해도 개별 관측의 ε를 모두 제거할 수 없는 이유는?", "ε에는 X만으로 미리 알 수 없는 관측의 불확실성이 포함되기 때문이다.", "모델은 데이터에서 f*를 근사하지만 같은 X에서도 달라질 수 있는 개별 오차까지 정확히 예측할 수는 없습니다. 이는 관계 학습으로 줄일 수 있는 오차와 구분되는 부분입니다.", undefined, ["ε에는 X만으로 미리 알 수 없는 관측의 불확실성이 포함되기 때문이다.", "ε는 모델의 학습이 끝나면 모든 관측에서 항상 1이 되기 때문이다.", "f*(X)는 label과 관계없이 임의의 범주만 반환하기 때문이다.", "학습을 시작하면 입력 X가 자동으로 데이터에서 삭제되기 때문이다."]),
  Q("ml-30", "ML 기초·검증", "객관식", "핵심", "F1 score에 대한 설명으로 옳은 것은?", "precision과 recall의 조화평균으로 두 지표의 균형을 반영한다.", "F1=2PR/(P+R)이며 precision과 recall 중 하나가 낮으면 함께 낮아집니다. 불균형 분류에서 두 관점을 함께 요약할 때 쓰지만 오류 비용에 따라 개별 지표도 확인해야 합니다.", undefined, ["precision과 recall의 조화평균으로 두 지표의 균형을 반영한다.", "accuracy와 specificity의 산술평균으로 전체 표본 수만 반영한다.", "TP와 TN의 차이를 계산해 회귀 오차의 원래 단위로 표현한다.", "threshold와 관계없이 모든 분류 모델에서 항상 같은 값을 갖는다."]),
  ...
  Q("nn-11", "회귀·신경망", "객관식", "핵심", "회귀 결과표에서 어떤 설명변수의 p-value가 0.05보다 매우 작을 때 해석으로 옳은 것은?", "해당 변수와 반응변수의 관계가 통계적으로 유의하다고 본다.", "p-value가 충분히 작으면 계수가 0이라는 가정과 관측된 결과가 잘 맞지 않는다는 뜻입니다. 따라서 다른 조건을 통제한 회귀모형 안에서 해당 변수의 계수가 0과 다를 근거가 있다고 해석합니다.", undefined, ["해당 변수와 반응변수의 관계가 통계적으로 유의하다고 본다.", "해당 변수를 빼도 R²가 반드시 같다고 본다.", "모든 관측치의 잔차가 0이라고 본다.", "분류 threshold를 자동으로 정했다고 본다."]),
  ...
  Q("nn-18", "회귀·신경망", "단답형", "핵심", "PyTorch의 nn.Module에서 forward를 오버라이드했을 때 이를 호출하는 매직 메서드 이름을 정확히 작성하시오.", "__call__", "nn.Module은 model(x)처럼 객체를 호출하면 __call__ 매직 메서드를 거쳐 forward를 실행합니다. 따라서 forward에는 입력에서 출력까지의 순전파 계산을 정의합니다."),
  ...
  Q("nn-24", "회귀·신경망", "객관식", "사고형", "다중선형회귀에 설명변수를 추가했을 때 일반적으로 작아지지 않는 값은?", "훈련 데이터에서의 R²", "새 설명변수를 넣은 모델은 기존 모델의 계수를 그대로 두고 새 계수만 0으로 둘 수 있습니다. 따라서 최소제곱으로 다시 맞춘 훈련 데이터 R²는 기존보다 작아지지 않습니다. 다만 새 변수가 일반화 성능을 높인다는 뜻은 아닙니다.", undefined, ["훈련 데이터에서의 R²", "훈련 데이터에서의 잔차표준오차 RSE", "검증 데이터에서의 예측오차", "개별 회귀계수의 표준오차"]),
  Q("nn-25", "회귀·신경망", "단답형", "기초", "실제값 y에서 예측값 ŷ를 뺀 y-ŷ를 한글로 작성하시오.", "잔차", "잔차는 실제값과 예측값의 차이 e=y-ŷ입니다. 모델이 각 관측치를 얼마나 높거나 낮게 예측했는지 보여 주며, 회귀 진단과 손실 계산의 출발점이 됩니다."),
  Q("nn-26", "회귀·신경망", "단답형", "기초", "잔차를 제곱해 모두 더한 잔차제곱합의 영문 약어를 대문자로 작성하시오.", "RSS", "RSS는 Residual Sum of Squares의 약어입니다. 잔차의 부호가 서로 상쇄되지 않도록 각각 제곱해 더하며, 값이 작을수록 관측값과 회귀선의 차이가 작습니다."),
  Q("nn-27", "회귀·신경망", "객관식", "핵심", "선형회귀의 최소제곱법이 계수를 정하는 핵심 기준은?", "잔차제곱합 RSS가 가장 작아지는 계수를 선택한다.", "각 데이터점의 실제값과 회귀선의 예측값 차이를 잔차라고 합니다. 최소제곱법은 이 잔차들을 제곱해 더한 RSS를 목적함수로 두고 가장 작은 계수를 찾습니다.", undefined, ["잔차제곱합 RSS가 가장 작아지는 계수를 선택한다.", "모든 예측값의 산술평균이 가장 커지는 계수를 선택한다.", "설명변수의 표본분산이 정확히 0이 되는 계수를 선택한다.", "개별 관측의 측정오차를 모두 삭제하는 계수를 선택한다."]),
  Q("nn-28", "회귀·신경망", "객관식", "핵심", "손실함수와 학습률의 역할을 올바르게 연결한 것은?", "손실은 예측 오차를 측정하고 학습률은 갱신 보폭을 정한다.", "손실함수는 예측이 얼마나 틀렸는지 하나의 값으로 계산합니다. 학습률은 optimizer가 gradient 방향으로 파라미터를 이동할 크기를 정하는 하이퍼파라미터입니다.", undefined, ["손실은 예측 오차를 측정하고 학습률은 갱신 보폭을 정한다.", "손실은 입력 순서를 바꾸고 학습률은 정답 label을 생성한다.", "손실은 파라미터를 갱신하고 학습률은 gradient를 직접 계산한다.", "손실은 test를 선택하고 학습률은 feature 개수를 자동 결정한다."]),
  Q("nn-29", "회귀·신경망", "객관식", "사고형", "SGD 또는 mini-batch 학습에 대한 설명으로 옳지 않은 것은?", "모든 문제에서 full-batch보다 반드시 더 빨리 수렴한다.", "mini-batch는 갱신당 계산량이 작고 자주 갱신할 수 있지만 gradient에 잡음이 있습니다. 데이터와 목적함수, batch 크기, 학습률에 따라 수렴 속도가 달라지므로 항상 더 빠르다고 단정할 수 없습니다.", undefined, ["한 번의 갱신에 전체보다 적은 표본을 사용할 수 있다.", "batch 구성에 따라 gradient가 어느 정도 흔들릴 수 있다.", "큰 데이터에서 갱신당 계산과 메모리 부담을 줄일 수 있다.", "모든 문제에서 full-batch보다 반드시 더 빨리 수렴한다."]),
  Q("nn-30", "회귀·신경망", "객관식", "기초", "단순선형회귀 Y=β₀+β₁X+ε에서 β₀의 의미는?", "X가 0일 때 회귀선이 예측하는 절편", "β₀는 회귀선이 y축과 만나는 절편으로 X=0일 때의 예측값입니다. β₁은 X가 한 단위 증가할 때 예측값의 변화량인 기울기입니다.", undefined, ["X가 0일 때 회귀선이 예측하는 절편", "X가 1 증가할 때 예측값이 변하는 기울기", "실제값에서 예측값을 뺀 개별 관측의 잔차", "학습 전에 사람이 정하는 optimizer의 학습률"]),
  ...
  Q("llm-19", "LLM·평가·안전", "객관식", "사고형", "합성 instruction 데이터를 그대로 대량 학습할 때의 위험으로 가장 적절한 것은?", "오류와 편향이 반복될 수 있다.", "합성 데이터는 빠르게 만들 수 있지만 생성 모델이 낸 사실 오류, 편향, 중복도 함께 들어갈 수 있습니다. 품질 점검과 중복 제거, 다양한 과제 구성이 필요한 이유입니다.", undefined, ["오류와 편향이 반복될 수 있다.", "학습 데이터의 크기가 자동으로 0이 된다.", "모든 응답이 항상 사실로 검증된다.", "사람의 품질 검토가 필요 없어지게 된다."]),
...
const hintExamQuestions: Question[] = hintExamTopics
...
export default function Home() {
  ...
  function startExam(
    ...
    let next: Question[];
    if (targetQuestions) {
      const targetCount = options?.allTargets
    ...
    setCurrent(0);
    setSeconds(
      options && "fixedMinutes" in options
        ? options.fixedMinutes === null
          ? null
          : options.fixedMinutes * 60
        : minutes === null
          ? null
          : minutes * 60,
    );
  ...
  return (
    <main>
      ...
      <section className="practice-section" id="practice">
        ...
        <section className="hint-exam-card" aria-labelledby="hint-exam-title">
          <div>
            <small>출제 안내 범위</small>
          ...
          <button
            onClick={() =>
              startExam(hintExamQuestions, { allTargets: true, fixedMinutes: 60 })
            }
          >
            힌트 모의고사 시작
          </button>
