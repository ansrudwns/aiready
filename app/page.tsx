...
const questionBank: Question[] = [
  ...
  Q("np-10", "NumPy·Pandas", "서술형", "사고형", "loc와 iloc의 차이와 각각이 적합한 사용 상황을 100자 이상으로 서술하시오.", "loc는 행·열의 라벨과 Boolean 조건을 기준으로 선택하므로 열 이름이나 조건으로 조회·수정할 때 적합하다. iloc는 0부터 시작하는 정수 위치를 기준으로 선택하므로 몇 번째 행과 열인지가 중요한 위치 기반 슬라이싱에 적합하다.", "loc는 label, iloc는 integer position을 기준으로 합니다. 이 차이를 무시하면 정수형 index에서 특히 혼동하기 쉽습니다."),
  ...
  Q("viz-08", "시각화·EDA", "서술형", "사고형", "타이타닉 EDA에서 전체 생존율만 보는 것이 불충분한 이유와 그룹별 분석 방법을 100자 이상으로 서술하시오.", "전체 생존율은 성별, 객실 등급, 나이, 요금 등 중요한 하위 집단의 차이를 숨길 수 있다. groupby나 pivot_table로 sex·pclass 등을 나누고 survived의 mean과 sum을 계산해 비율과 인원을 함께 보며, 그룹 크기와 결측치도 확인해야 한다.", "전체 평균만으로는 심플슨의 역설과 같은 집단 차이를 놓칠 수 있습니다. 그룹별 생존율과 표본 수를 같이 비교하고 시각화로 패턴을 확인해야 합니다."),
  ...
  Q("ml-08", "ML 기초·검증", "서술형", "사고형", "정확도 99%인 질병 분류기가 실제로는 유용하지 않을 수 있는 상황을 설명하고, 함께 봐야 할 지표를 100자 이상으로 서술하시오.", "환자가 1%인 데이터에서 모두 정상으로 예측해도 accuracy는 99%가 되지만 환자를 하나도 찾지 못한다. 혼동행렬의 TP·FP·FN·TN을 확인하고 환자 누락이 중요하면 recall, 양성 예측의 신뢰도는 precision, 균형은 F1과 ROC-AUC로 평가해야 한다.", "클래스 불균형이 크면 accuracy가 다수 클래스 예측만을 반영할 수 있습니다. 비용이 큰 오류가 FP인지 FN인지에 따라 지표와 threshold를 선택합니다."),
  ...
  Q("ml-10", "ML 기초·검증", "객관식", "핵심", "Y=f*(X)+ε에서 E[ε]=0이고 ε가 X와 독립이라는 가정의 의미로 옳은 것은?", "주어진 X에서 체계적인 평균 관계를 f*(X)가 나타낸다.", "잡음의 조건부 평균이 0이면 X가 주어졌을 때 Y의 평균 관계를 f*가 담당합니다. 개별 관측치의 오차가 모두 0이라는 뜻은 아닙니다.", undefined, ["주어진 X에서 체계적인 평균 관계를 f*(X)가 나타낸다.", "모든 관측치가 f*(X)와 정확히 같다.", "ε는 항상 양수다.", "X와 Y는 반드시 독립이다."]),
  ...
  Q("nn-03", "회귀·신경망", "객관식", "핵심", "로지스틱 회귀의 sigmoid 출력에 대한 설명으로 옳은 것은?", "0과 1 사이의 값으로 양성 확률로 해석할 수 있다.", "sigmoid는 선형 점수 z를 0과 1 사이로 압축합니다. 이진 로지스틱 회귀에서 p(y=1|x)로 해석하고 threshold를 적용해 class를 결정합니다.", undefined, ["실수 전체를 그대로 출력한다.", "0과 1 사이의 값으로 양성 확률로 해석할 수 있다.", "항상 0 또는 1만 출력한다.", "다중 분류의 클래스 수를 결정한다."]),
  ...
  Q("nn-05", "회귀·신경망", "객관식", "핵심", "경사하강법의 파라미터 갱신식으로 옳은 것은?", "θ ← θ - η∇L(θ)", "gradient는 손실이 가장 빠르게 증가하는 방향을 가리킵니다. 손실을 줄이려면 현재 파라미터에서 learning rate×gradient를 빼야 합니다.", undefined, ["θ ← θ + η∇L(θ)", "θ ← θ - η∇L(θ)", "θ ← η / ∇L(θ)", "θ ← L(θ)"]),
  ...
  Q("nn-11", "회귀·신경망", "객관식", "핵심", "잔차표준오차 RSE에 대한 설명으로 옳은 것은?", "잔차의 전형적인 크기를 목표값과 같은 단위로 나타낸다.", "RSE는 RSS를 잔여 자유도로 나눈 값의 제곱근입니다. 잔차의 전형적 규모를 반응변수의 원래 단위로 해석하며 계수 불확실성과 모델 적합도를 판단하는 데 쓰입니다.", undefined, ["잔차의 전형적인 크기를 목표값과 같은 단위로 나타낸다.", "분류 정확도와 항상 같다.", "파라미터 수와 무관하게 RSS 자체만 뜻한다.", "0과 1 사이의 확률만 반환한다."]),
  ...
  Q("nn-15", "회귀·신경망", "객관식", "고난도", "ReLU 신경망에서 깊이를 늘릴 때 나타날 수 있는 표현력 변화로 옳은 것은?", "비슷한 파라미터 수에서도 조합으로 더 많은 선형 영역을 만들 수 있다.", "깊은 구조는 앞 층에서 만든 꺾임을 뒤 층이 다시 조합하여 선형 영역 수를 효율적으로 늘릴 수 있습니다. 깊이가 항상 일반화 성능을 보장하는 것은 아닙니다.", undefined, ["비슷한 파라미터 수에서도 조합으로 더 많은 선형 영역을 만들 수 있다.", "ReLU를 쓰면 층 수와 관계없이 항상 직선 하나만 표현한다.", "깊어질수록 파라미터가 반드시 0개가 된다.", "깊이는 계산 그래프와 무관하다."]),
  ...
  Q("nn-20", "회귀·신경망", "서술형", "사고형", "선형회귀와 로지스틱 회귀의 출력·가정·대표 손실 차이를 100자 이상으로 서술하시오.", "선형회귀는 입력의 선형결합으로 연속값의 조건부 평균을 예측하며 보통 제곱오차 또는 정규오차 가정의 음의 로그우도를 사용한다. 로지스틱 회귀는 선형 점수를 sigmoid에 통과시켜 이진 class의 확률을 만들고 Bernoulli 우도에 대응하는 binary cross-entropy를 최소화한다. 확률에 threshold를 적용하면 class 예측이 된다.", "이름에 회귀가 공통으로 들어가도 목표 변수와 출력 범위가 다릅니다. 선형 점수 이후의 연결함수와 likelihood가 달라지므로 손실과 해석도 구분됩니다."),
  ...
  Q("nlp-09", "NLP·Transformer", "서술형", "고난도", "RNN과 Transformer의 시퀀스 처리 차이를 병렬성과 장기 의존성 관점에서 100자 이상으로 서술하시오.", "RNN은 이전 hidden state를 다음 시점에 전달하므로 시퀀스를 순차적으로 처리해 병렬화가 제한되고 긴 거리에서 gradient 소실이 발생할 수 있다. Transformer는 self-attention으로 모든 위치 사이의 관계를 직접 계산해 학습 병렬성과 장기 관계 모델링을 개선하지만 길이 N에 대한 attention 메모리 비용이 일반적으로 O(N²)이다.", "두 구조는 정보 전달 경로와 계산량 특성이 다릅니다. Transformer가 항상 더 좋다고만 쓰지 말고 장단점을 함께 설명해야 합니다."),
  ...
  Q("llm-09", "LLM·평가·안전", "서술형", "사고형", "LLM 환각의 의미와 실제 시스템에서의 완화 방법을 100자 이상으로 서술하시오.", "LLM 환각은 모델이 사실과 다르거나 근거가 없는 내용을 자신 있게 생성하는 현상이다. 최신·전문 정보는 RAG로 검색 근거를 제공하고 근거 표시와 후처리 검증을 요구하며, 중요한 의사결정은 사람이 원문을 확인해야 한다. 평가용 테스트셋으로 사실성을 계속 측정하고 모델이 모르는 경우를 표현하게 하는 지시도 사용한다.", "프롬프트만으로 환각을 0으로 만들 수는 없습니다. 근거 검색, 검증, 평가, 사람 검토를 겹친 시스템 설계가 필요합니다."),
  ...
  Q("llm-20", "LLM·평가·안전", "서술형", "고난도", "RAG와 fine-tuning이 해결하는 문제가 어떻게 다른지, 최신 사내 지식 질의 시스템에서는 어떤 조합이 적절한지 100자 이상으로 서술하시오.", "RAG는 질의 시점에 외부 지식을 검색해 context에 넣으므로 자주 바뀌는 사실을 갱신하고 근거를 연결하는 데 적합하다. fine-tuning은 모델 weight를 바꿔 응답 형식·용어·행동 패턴을 안정화하지만 최신 사실을 안전하게 저장하는 방법은 아니다. 사내 시스템은 권한이 적용된 검색과 RAG로 지식을 제공하고 필요하면 SFT로 형식을 맞춘 뒤, 검색 실패와 생성 사실성을 별도로 평가하는 조합이 적절하다.", "검색은 지식 공급, fine-tuning은 행동 적응이라는 역할 분리가 핵심입니다. 두 방법 모두 접근제어, 문서 품질, 평가와 사람 검토를 자동으로 해결하지는 않습니다."),
];
    ...
    <main>
      ...
      <section className="practice-section" id="practice">
        ...
        <div className="exam-guidance" aria-label="과목평가 답안 유의사항">
          ...
          <span>서술형은 결론과 이유를 빠짐없이 100자 이상 작성한 뒤 모범답안과 비교합니다.</span>
